// Redesigned by t.me/Ognisty321 from https://github.com/OgnistyPoland/test123 which was written by someone else, credits are given on Source Page.
// v2.0.10
// Initialize the page
function init() {
...
<br>
<footer class="text-muted"> <div class="container"> <p class="float-right"> <a href="#">Powrót do góry</a> </p> ${UI.credit ? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://github.com/OgnistyPoland/test123" target="_blank">Ognisty321</a>, based on Open Source Softwares.</p>' : ''} <p>© ${UI.copyright_year} - <a href=" ${UI.company_link}" target="_blank"> ${UI.company_name}</a></p> </div> </footer>
  `;
	$('body').html(html);
}
...
function nav(path) {
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item">
        <a class="nav-link" href="/${cur}:/">Strona główna</a>
      </li>`;
	...
	html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Bieżąca ścieżka</a><div class="dropdown-menu" aria-labelledby="navbarDropdown"><a class="dropdown-item"  href="/${cur}:/ ">> Strona główna</a>`;
    ...
    <a class="nav-link" href="${UI.contact_link}" target="_blank">Discord</a>
	<li class="nav-item">
	<a class="nav-link" href="https://www.cda.pl/shisha-subs" target="_blank">CDA</a>
	<li class="nav-item">
	<a class="nav-link" href="https://www.facebook.com/sziszasubs" target="_blank">FB</a>
  </li>`;
...
<input class="form-control mr-sm-2" name="q" type="search" placeholder="Czego szukasz?" aria-label="Search" value="${search_text}" required>
<button class="btn ${UI.dark_mode ? 'btn-secondary' : 'btn-outline-success'} my-2 my-sm-0" onclick="if($('#search_bar').hasClass('mdui-textfield-expanded') && $('#search_bar_form>input').val()) $('#search_bar_form').submit();" type="submit">Szukaj</button>
</form>
</div>
</nav>
`;
...
function list(path) {
  ...
  `;
	...
	$('#list').html(`<div class="d-flex justify-content-center"><div class="spinner-border m-5 text-primary" role="status"><span class="sr-only">Wczytywanie...</span></div></div>`);
	$('#readme_md').hide().html('');
	$('#head_md').hide().html('');
			...
			if (window.scroll_status.event_bound !== true) {
				...
				$(window).on('scroll', function () {
					...
					if (scrollTop + windowHeight > scrollHeight - (Os.isMobile ? 130 : 80)) {
						...
						$(`<div id="spinner" class="d-flex justify-content-center"><div class="spinner-border m-5 text-primary" role="status"><span class="sr-only">Wczytywanie...</span></div></div>`)
...
function render_search_result_list() {
  ...
  <div class="container"><br>
  <div class="card">
  <h5 class="card-header">Wyniki wyszukiwania</h5>
  <div id="list" class="list-group">
  </div>
  </div>
  <div class="card">
	...
	$.post(`/${cur}:id2path`, {
		...
		if (data) {
			...
			dialog = mdui.dialog({
				...
				content: `<a class="btn btn-info" href="${hrefurl}">Otwórz</a> <a class="btn btn-secondary" href="${hrefurl}" target="_blank">Otwórz w nowej karcie</a> <button class="btn btn-danger" mdui-dialog-cancel>Zamknij</button><script>dialog.addEventListener('cancel.mdui.dialog', function () {
  console.log('cancel');
});</script>`,
				history: false,
				modal: true,
...
<p class="card-text text-center"><a href="${href}" class="btn btn-primary">Pobierz</a></p><br>`;
	$('#content').html(content);
}
...
<p class="card-text text-center"><a href="${href}" class="btn btn-primary">Pobierz</a></p><br>
<script src="https://cdn.jsdelivr.net/gh/OgnistyPoland/szyszka/js/ace/1.4.7/ace.js"></script>
<script src="https://cdn.jsdelivr.net/gh/OgnistyPoland/szyszka/js/ace/1.4.7/ext-language_tools.js"></script>`;
	$('#content').html(content);
...
function file_video(path) {
  ...
  <script>
   const player = new Plyr('#vplayer');
	...
	<p class="card-text text-center"><a href="${url}" class="btn btn-primary">Pobierz</a></p><br>
  </div>
  </div>
  `;
	$('#content').html(content);
